...
def upgrade() -> None:
    ...
    # 1) Create ENUM type (PostgreSQL)
    file_category_enum.create(bind, checkfirst=True)

    # 2) Add nullable column first
    op.add_column("files", sa.Column("category", file_category_enum, nullable=True))

    # 3) Backfill existing rows
    op.execute("UPDATE files SET category = 'other' WHERE category IS NULL")

    # 4) Enforce NOT NULL
    op.alter_column("files", "category", existing_type=file_category_enum, nullable=False)
